...
export const uploadEmployees = [
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // Get employee codes from the uploaded data
      const uploadedCodes = new Set(employees.map(e => e.employee_code));
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      for (const row of data as any[]) {
          ...
          if (typeof dateValue === 'string') {
            ...
            if (mmddyyyyMatch) {
              ...
              if (ddmmyyyyMatch) {
                ...
                // Try other formats via helper function
                date = excelDateToDateString(dateValue);
              }
            }
          } 
        ...
        department = normalizedRow['Phòng Ban'] || 
      ...
      // Đánh dấu tất cả dữ liệu cũ là archived (chuyển sang Lịch sử)
      // Dữ liệu mới upload sẽ có is_archived = 0 (hiển thị ở Báo cáo)
      // QUAN TRỌNG: Archive TRƯỚC khi insert dữ liệu mới
      console.log(`\n📦 Đang đánh dấu tất cả dữ liệu cũ là archived...`);
      
      // Lấy thời gian hiện tại để đánh dấu dữ liệu mới
      const currentUploadTime = new Date().toISOString();
      
      // Archive tất cả dữ liệu có is_archived = 0 (dữ liệu chưa được archive)
      // Không dùng null vì Prisma không hỗ trợ null trong where clause cho SQLite
      // Thay vào đó, chỉ archive những record có is_archived = 0
      const archiveResult = await prisma.timekeepingRecord.updateMany({
        where: {
          is_archived: 0, // Chỉ archive dữ liệu chưa được archive (is_archived = 0)
        },
        data: {
          is_archived: 1, // Đánh dấu là dữ liệu cũ (chuyển sang Lịch sử)
        },
      });
      console.log(`✓ Đã chuyển ${archiveResult.count} bản ghi cũ sang Lịch sử (is_archived = 1)`);
      
      // Log để debug
      const totalRecords = await prisma.timekeepingRecord.count();
      const archivedRecords = await prisma.timekeepingRecord.count({ where: { is_archived: 1 } });
      const newRecordsBefore = await prisma.timekeepingRecord.count({ 
        where: { 
          is_archived: 0,
        } 
      });
      console.log(`📊 Trước khi insert: Tổng ${totalRecords}, Đã archive: ${archivedRecords}, Dữ liệu mới: ${newRecordsBefore}`);
      ...
      if (employeeCodes.length > 0) {
        const employees = await prisma.employee.findMany({
          where: {
            employee_code: { in: employeeCodes },
          },
          select: { id: true, employee_code: true },
        });
        
        employees.forEach(emp => {
          employeeMap.set(emp.employee_code, emp.id);
        });
      }
      
      // Batch insert records (SQLite doesn't support createMany well, so we'll do in batches)
      const BATCH_SIZE = 50; // Smaller batches for SQLite
      let insertedCount = 0;
      const insertedRecords: any[] = [];
      
      console.log(`\nStarting batch insert of ${records.length} records...`);
      ...
      for (let i = 0; i < records.length; i += BATCH_SIZE) {
        ...
        for (const recordData of batch) {
          try {
            ...
            skippedRows++;
            if (error.code === 'P2002') {
              // Duplicate record - log first few
              if (skippedRows <= 5) {
                console.log(`⚠️ Duplicate record skipped: ${recordData.employee_code} - ${recordData.date}`);
              }
            } else {
              // Log all errors for first 20 records
              if (skippedRows <= 20) {
                console.error(`❌ Error creating timekeeping record ${insertedCount + skippedRows}:`, {
                  error: error.message,
                  code: error.code,
        ...
        if ((i + BATCH_SIZE) % 500 === 0 || i + BATCH_SIZE >= records.length) {
          console.log(`Processed ${Math.min(i + BATCH_SIZE, records.length)}/${records.length} records... (Inserted: ${insertedCount})`);
        }
      }
