  ...
  return (
    <div className="rounded-lg border border-border bg-card flex flex-col items-center justify-center py-16 px-6 text-center">
      <p className="text-muted-foreground text-sm">
        {comingSoon
          ? t('deptMenu.deptComingSoon', { dept })
          : t('deptMenu.selectFromPopup', { dept })}
      </p>
    </div>
  );
}
...
/** Dịch tiêu đề thông báo từ tiếng Việt sang tiếng Hàn khi cần */
function translateNoticeTitle(
  title: string,
  language: 'vi' | 'ko',
  t: (k: string, p?: Record<string, string | number>) => string
): string {
  if (language !== 'ko' || !title) return title;
  const m1 = title.match(/Đã đồng bộ (\d[\d,.]*) nhân viên thời vụ/);
  if (m1) return t('dashboard.syncedTempEmployees', { n: m1[1].replace(/,/g, '') });
  const m2 = title.match(/Đã đồng bộ (\d[\d,.]*) nhân viên chính thức/);
  if (m2) return t('dashboard.syncedOfficialEmployees', { n: m2[1].replace(/,/g, '') });
  const m3 = title.match(/(\d[\d,.]*) nhân viên đi trễ hôm nay/);
  if (m3) return t('dashboard.employeesLateToday', { n: m3[1].replace(/,/g, '') });
  if (/Upload dữ liệu thành công/i.test(title)) return t('dashboard.uploadSuccess');
  return title;
}
...
const Index = () => {
  ...
  const formatDateDisplay = (dateStr: string) => {
    ...
    return date.toLocaleDateString(language === 'ko' ? 'ko-KR' : 'vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' });
  };
  ...
  const headerDateLabel =
    filterMode === 'month'
      ...
      ? `${t('dashboard.oneDay')} ${formatDateDisplay(dateUsedFromApi)} (${t('dashboard.latestData')})`
      : filterMode === 'single'
  ...
  return (
    <div>
      ...
      <div className="w-full">
        ...
        {activeDeptTab === 'accounting' && (
          selectedFunction === null ? (
            <DeptPlaceholder dept={t('deptMenu.accounting')} t={t} />
          ) : (
            <div className="space-y-4">
        ...
        {activeDeptTab === 'administration' && (
          selectedFunction === null ? (
            <DeptPlaceholder dept={t('deptMenu.administration')} t={t} />
          ) : (
            <div className="space-y-4">
              <Button variant="ghost" size="sm" onClick={() => setSelectedFunction(null)} className="gap-2">
                <ArrowLeft className="w-4 h-4" />
        ...
        {/* Công vụ / Mua hàng / EHS */}
        {(activeDeptTab === 'congvu' || activeDeptTab === 'muahang' || activeDeptTab === 'ehs') && (
          <DeptPlaceholder dept={activeDeptTab === 'congvu' ? t('deptMenu.congvu') : activeDeptTab === 'muahang' ? t('deptMenu.muahang') : t('deptMenu.ehs')} comingSoon t={t} />
        )}

        {/* Nhân sự */}
        {activeDeptTab === 'hr' && (
          selectedFunction === null ? (
            <DeptPlaceholder dept={t('deptMenu.hr')} t={t} />
          ) : HR_REPORT_INLINE_IDS.has(selectedFunction) ? (
            <div className="space-y-4">
              <Button variant="ghost" size="sm" onClick={() => setSelectedFunction(null)} className="gap-2">
                <ArrowLeft className="w-4 h-4" />
                {t('dashboard.goBack')}
              ...
              <div className="rounded-lg border border-border bg-card overflow-hidden">
                <div className="max-h-[70vh] overflow-y-auto">
                  <HrReportContent reportType={selectedFunction} compact />
                </div>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        ...
        />
        <StatCard
          title={`HR - ${t('dashboard.hrNewEmployees')}`}
          value={hrAttendanceCountValue}
          icon={Users}
          variant="success"
        />
        <StatCard
          title={`HR - ${t('dashboard.hrInsuranceEmployees')}`}
