...
export interface MenuTreeItem {
  id: string;
  label: string;
  labelKey?: string; // e.g. 'deptMenu.overview' for i18n
  icon?: LucideIcon;
}
          ...
          {items.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-sm font-medium text-center line-clamp-2">{item.labelKey ? t(item.labelKey) : item.label}</span>
              </button>
            );
          })}
        </div>
