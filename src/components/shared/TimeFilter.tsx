          ...
          <div className="space-y-2">
            ...
            <div className="pt-1 border-t border-sidebar-hover/50">
              ...
              <div className="flex flex-wrap gap-x-3 gap-y-1">
                {[
                  { key: 'chinhThuc' as const, label: 'Chính thức' },
                  { key: 'thoiVu' as const, label: 'Thời vụ' },
                ].map(({ key, label }) => (
                  <label key={key} className="flex items-center gap-1.5 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={periodFilters[key]}
                      onChange={(e) => setPeriodFilters((p) => ({ ...p, [key]: e.target.checked }))}
                      className="w-3.5 h-3.5 rounded border-sidebar-hover text-primary"
                    />
            ...
            <div>
              <Label className="text-xs text-sidebar-foreground/80 mb-1.5 block">Giới tính</Label>
              <div className="flex flex-wrap gap-x-3 gap-y-1">
                {[
                  { key: 'nam' as const, label: 'Nam' },
                  { key: 'nu' as const, label: 'Nữ' },
                ].map(({ key, label }) => (
                  <label key={key} className="flex items-center gap-1.5 cursor-pointer">
                    <input
                      type="checkbox"
